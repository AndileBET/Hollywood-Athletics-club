...
export function secondsToPace(secondsPerKm) {
  ...
  const minutes = Math.floor(Number(secondsPerKm) / 60);
  const seconds = Math.round(Number(secondsPerKm) % 60);
...
function toUiActivity(row) {
  ...
  const movingSeconds = Number(row.moving_time_seconds || 0);
  const rawDuration = movingSeconds ? secondsToDuration(movingSeconds) : row.duration || secondsToDuration(0);
  ...
  return {
    ...
    points: Number(row.points ?? row.points_earned ?? calculateActivityPoints(distance)),
